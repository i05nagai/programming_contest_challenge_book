...
TEST(bipartite_matching_test, BipartiteMatchingFordFulkersonSimple1)
{
...
TEST(bipartite_matching_test, BipartiteMatchingSimple1)
{
  const int num_computer = 3;
  const int num_work = 3;
  bool** bipartite_graph = new bool*[num_computer];
  ...
  const int actual = BipartiteMatching(bipartite_graph, num_computer, num_work);
  const int expect = 3;
  EXPECT_EQ(expect, actual);
}
} } // namespace algorithm { namespace graph {
